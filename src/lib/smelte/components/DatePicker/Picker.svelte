...
<script lang="ts">
  ...
  export let paginatorProps = {
    ...
    iconClasses: (c: string): string => c.replace('p-4', ''),
    disabledClasses: (c: string): string =>
      c
  ...
  $: isCurrentYear = new Date().getFullYear() === temp.getFullYear();
  ...
  function dayIsSelected(day) {
    ...
    return (
      value.getDate() === day &&
      temp.getFullYear() === value.getFullYear() &&
