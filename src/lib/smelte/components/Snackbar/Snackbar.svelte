...
<script lang="ts">
  ...
  export let inProps: {
    ...
    start?: number;
    opacity?: number;
  } = { duration: 100, easing: quadIn };
  /** Out-animation props. "Fade"-Animation */
  export let outProps: {
    duration?: number;
    easing?: typeof quadOut;
    delay?: number;
  } = { duration: 100, easing: quadOut, delay: 150 };
  ...
  /** Hash. Bindable. */
  export let hash: string | boolean | null = false;
  ...
  export let classes: string | ((s: string) => string) = wrapperDefault;
...
{#if value && running === hash}
  <div class="fixed w-full h-full top-0 left-0 z-30 pointer-events-none">
    <div class={wClasses}>
      <div bind:this={node} in:scale={inProps} out:fade={outProps} on:click={() => (value = false)}>
        <slot />
        {#if !noAction}
          <Spacer />
          <slot name="action">
